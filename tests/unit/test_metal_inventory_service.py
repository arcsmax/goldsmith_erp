...
from goldsmith_erp.db.models import MaterialUsage, MetalPurchase
...
class TestMetalPurchaseCreation:
    ...
    async def test_create_purchase_success(self, db_session):
        ...
        purchase = await MetalInventoryService.create_purchase(
            db_session, purchase_data
        )
    ...
    async def test_create_purchase_calculates_price_per_gram(self, db_session):
        ...
        purchase_data = MetalPurchaseCreate(
            metal_type=MetalType.SILVER_925, weight_g=500.0, price_total=250.00
        )
    ...
    async def test_create_purchase_rounds_price(self, db_session):
        ...
        purchase_data = MetalPurchaseCreate(
            metal_type=MetalType.GOLD_18K, weight_g=333.0, price_total=15000.00
        )
...
class TestMaterialAllocation:
    ...
    async def test_allocate_fifo_multiple_batches(
        self, db_session, multiple_metal_purchases
    ):
    ...
    async def test_allocate_lifo_uses_newest_first(
        self, db_session, multiple_metal_purchases
    ):
    ...
    async def test_allocate_specific_insufficient_weight(
        self, db_session, sample_metal_purchase
    ):
    ...
    async def test_allocate_insufficient_inventory_total(
        self, db_session, sample_metal_purchase
    ):
...
class TestMaterialConsumption:
    ...
    async def test_consume_material_reduces_inventory(
        self, db_session, sample_metal_purchase, sample_order
    ):
    ...
    async def test_consume_material_creates_usage_record(
        self, db_session, sample_metal_purchase, sample_order
    ):
    ...
    async def test_consume_material_updates_order_cost(
        self, db_session, sample_metal_purchase, sample_order
    ):
...
class TestInventoryStatistics:
    ...
    async def test_get_inventory_summary(
        self, db_session, sample_metal_purchase, silver_purchase
    ):
        ...
        # Check gold summary
        gold_summary = next(
            s for s in stats.metal_types if s.metal_type == MetalType.GOLD_18K
        )
    ...
    async def test_get_inventory_summary_low_stock_alert(self, db_session):
        ...
        assert any(
            "platinum_950" in alert and "25" in alert
...
class TestListPurchases:
    ...
    async def test_list_purchases_filter_by_metal_type(
        ...
        gold_purchases = await MetalInventoryService.list_purchases(
            db_session, metal_type=MetalType.GOLD_18K
        )
    ...
    async def test_list_purchases_exclude_depleted(
        ...
        purchases = await MetalInventoryService.list_purchases(
            db_session, include_depleted=False
        )

        assert len(purchases) == 0

    async def test_list_purchases_include_depleted(
        self, db_session, sample_metal_purchase
    ):
        ...
        purchases = await MetalInventoryService.list_purchases(
            db_session, include_depleted=True
        )
