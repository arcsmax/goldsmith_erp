...
# W2-05 re-implementation note: conversion now refuses a quote without an
# agreed price (A3.1), so the double-conversion and drift cases carry one
# priced line; approval now records how the customer agreed (DOM-11d).
_PRICED_LINE = [
    QuoteLineItemCreate(
    ...
    async def test_order_reassigned_to_other_customer_after_quote_creation(
        ...
        quote = await QuoteService.create_quote(
            ...
            QuoteCreate(
                customer_id=customer_a.id,
                order_id=order.id,
                additional_line_items=_PRICED_LINE,
            ),
            user,
        )
        await QuoteService.approve_quote(
