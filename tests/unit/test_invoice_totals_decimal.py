"""
A1 — Adversarial rounding tests (copied from worktree-agent-a0c7685dedac160fc tests/adversarial/test_money_rounding.py) for the invoice/quote money path.
...
class TestGrossMinusNetEqualsVatAtEveryBoundary:
    """A1 fix: the persisted floats satisfy total - subtotal == tax_amount to
    the cent for every half-cent boundary, and match the Decimal reference."""

    @pytest.mark.parametrize("tax_rate", [0.0, 7.0, 19.0])
    @pytest.mark.parametrize("cents_times_ten", range(0, 2000, 5))
    def test_invariant(self, tax_rate: float, cents_times_ten: int) -> None:
        price = cents_times_ten / 1000  # 0.000, 0.005, 0.010, ... 1.995
        items = [(1, price), (3, price)]
        got = InvoiceService.calculate_totals(
            [_item(q, p) for q, p in items], tax_rate=tax_rate
        )

        assert got == _decimal_reference(items, tax_rate)
        assert Decimal(str(got["total"])) - Decimal(str(got["subtotal"])) == Decimal(
            str(got["tax_amount"])
        )
