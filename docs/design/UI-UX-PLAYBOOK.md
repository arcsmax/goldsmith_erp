...
*Implemented in src/ui (W4-02):* `Button.tsx` exports `Button`, `ButtonLink` and `IconButton`. Differences from the sketch: `icon` is an `IconName` from `src/ui/Icon.tsx` (inline SVG set, stand-in until an icon package is adopted); extra `block?: boolean` (full width); `ButtonLink` = same props on a react-router `<Link>` (`to`, ...). `IconButton` also accepts button attributes (`onClick`, `disabled`); a missing or blank `label` throws a dev-only assertion. Styles in `src/styles/ui.css` (one stylesheet instead of per-folder CSS).
...
*Implemented in src/ui (W4-02):* `DeadlineChip.tsx`, props as sketched plus `size?: 'md' | 'lg'`; `describeDeadline(deadline, now)` is exported. Calendar days: more than 3 days ● neutral "in N Tagen"; 0 to 3 days ▲ waiting ("heute fällig", "morgen fällig", "in 2 Tagen"); past ■ danger "N Tage überfällig"; `null` shows "Keine Frist". The exact date sits in `<time title="Frist: TT.MM.JJJJ">`. StatusBadge and `status.ts` are a separate item.
...
*Implemented in src/ui (W4-02):* `Modal.tsx` exports `Modal`, `Sheet` (same props, bottom sheet at every width) and `Dialog` (`{ open, title, message, confirmLabel?, cancelLabel?, variant?: 'default' | 'danger', loading?, onConfirm, onCancel }`, role `alertdialog`, focus on "Abbrechen"). `ModalProps` as sketched plus `className?`; `initialFocusRef` accepts `RefObject<HTMLElement | null>`. Portal plus tested focus trap (happy-dom has no `showModal()`); only the top-most modal handles Escape and Tab. The dirty question is inline in the footer ("Änderungen verwerfen?" / "Weiter bearbeiten" / "Verwerfen") via `src/lib/useDirtyGuard.ts` (`useDirtyGuard(isDirty, onClose)` → `{ isConfirming, requestClose, discard, keepEditing }`, also warns on unload). `PromptDialog.tsx` replaces `window.prompt`: `{ open, title, label, description?, help?, defaultValue?, placeholder?, confirmLabel?, cancelLabel?, required?, validate?, inputMode?, multiline?, maxLength?, onSubmit(value), onCancel }`, plus `usePromptDialog()` → `{ prompt(options): Promise<string | null>, dialog }`. The existing `ConfirmDialog`/`useConfirm` are unchanged (migration later).
...
*Implemented in src/ui (W4-02):* `Field.tsx`, props as sketched plus `suffix?: string` (unit, also announced as "Einheit: …") and `className?`. The "Pflichtfeld" marker is visible and `aria-hidden` (aria-required carries it). An `id` already on the child is kept.
...
*Implemented in src/ui (W4-02):* `DataTable.tsx` and `ListCard.tsx`. Differences: the table shows from 600px (not 1024px), cards below; `Column<T>` adds `sortable?`; `DataTableProps<T>` = `{ rows, columns, getRowKey, rowHref?, caption, showCaption?, state?, empty?, sort?: { key, direction: 'asc' | 'desc' }, onSortChange?, cardTitle?, cardMeta?, cardBadges?, cardThumbnail?, className? }`. The first column is the row link; sortable headers are buttons inside `<th aria-sort>`. `hideBelow: 'tablet'` also drops the column from the phone card. `ListCard` = `{ href?, title, meta?, badges?, thumbnail?, details?, className? }`. StatusBadge in rows: TODO once the StatusBadge item lands.
### 4.6 Card
...
*Implemented in src/ui (W4-02):* `Card.tsx` = `{ title?, headingLevel?: 2 | 3, action?, tone?: 'info' | 'waiting' | 'danger' | 'done', className?, children }`. With a title it is a region labelled by its heading.
### 4.7 EmptyState
...
*Implemented in src/ui (W4-02):* `EmptyState.tsx` = `{ icon?: IconName, title, body?, action?: ReactNode, secondaryAction?: ReactNode, headingLevel?: 2 | 3, className? }` (action is a Button or ButtonLink node).
...
*Implemented in src/ui (W4-02):* `PageState.tsx` = `{ state: PageStateValue, skeleton?: 'list' | 'detail' | 'cards', skeletonCount?, empty?: EmptyStateProps, className?, children }`; `SKELETON_DELAY_MS = 300`. Error text: pass `getErrorMessage(err)` from `src/lib/errors.ts` (understands the `{detail, code, extra}` envelope, legacy `{detail: {code, message}}`, FastAPI 422 lists, network/timeout; also `getErrorCode`, `getErrorStatus`). No separate LoadingState component.
### 4.9 Toast
...
*Implemented in src/ui (W4-02):* `PageHeader.tsx` = `{ title, meta?, back?: { to, label }, primaryAction?: ReactNode, secondaryActions?: ReactNode, stickyPrimary?: boolean (default true), className? }`. `back` uses `to` (router path) instead of `href`; `primaryAction` is a slot (a Button node) rather than ButtonProps; below 600px it becomes a fixed bottom bar and `main` gets bottom padding.
...
*Implemented in src/ui (W4-02):* `Tabs.tsx` exports `Tabs` (`{ label, tabs: { id, label, panel? }[], selectedId, onSelect, renderPanels?, className? }`, automatic activation, ArrowLeft/Right wrap, Home/End), `useTabParam(ids, defaultId, param = 'tab')` → `[tab, setTab]` (keeps other query params, `replace`), and `TabBar` (`{ label, items: { to, label, icon, prominent? }[], className? }`, NavLink with `aria-current="page"`, fixed below 1024px). All primitives are shown in every state at `/dev/ui` (development builds only).
