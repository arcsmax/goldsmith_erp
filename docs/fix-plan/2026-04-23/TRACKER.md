...
|----|-------|--------|------------|--------|------|------------|------|
| A1 | Register `AuditLoggingMiddleware` + populate `request.state.user` | M | BE + GDPR | ⏳ pending | 1 | — | [A1-audit-middleware.md](A1-audit-middleware.md) |
| A2 | Merge duplicate `Permission` / `require_permission` systems | M | BE + SEC | ⏳ pending | 1 | — | [A2-permission-merge.md](A2-permission-merge.md) |
| A3 | Lock down `/users/register` | S | BE + SEC | 🛑 blocked | 2 | A2; product decision | [A3-register-endpoint.md](A3-register-endpoint.md) |
| A4 | Compose password defaults + bind DB to 127.0.0.1 (dev) | S | OPS | ⏳ pending | 1 | — | [A4-compose-defaults.md](A4-compose-defaults.md) |
| A5 | Add top-level + MainLayout `ErrorBoundary` | S | FE | ⏳ pending | 1 | — | [A5-error-boundary.md](A5-error-boundary.md) |
| A6 | `CustomerPortalPage` raw fetch leaks cookies | S | FE + SEC | ⏳ pending | 1 | — | [A6-portal-fetch.md](A6-portal-fetch.md) |
...
| B1+B2 | Bump axios (CVE-2026-25639) + react-router-dom (CVE-2026-22029 + CVE-2026-21884) | S | DEP + FE | ⏳ pending | 1 | — | [B1-B2-cve-bumps.md](B1-B2-cve-bumps.md) |
...
| F2 | Alembic upgrade→downgrade→upgrade smoke test in CI | S | CI | ⏳ pending | 1 | — | [F2-downgrade-smoke.md](F2-downgrade-smoke.md) |
...
Additionally from Group A-adjacent: **D3** (time_tracking.py missing `@require_permission`, P0, S effort, from report 01) — tiny, included in Wave 1 since it depends only on `core.permissions` being canonical (in flight via A2).

| ID | Title | Effort | Owner area | Status | Wave | Blocked-by | Spec |
|----|-------|--------|------------|--------|------|------------|------|
| D3 | Add `@require_permission(Permission.TIME_VIEW_OWN)` to `GET /time-tracking/user/{user_id}` | S | BE + SEC | ⏳ pending | 1 | must land AFTER A2 merges into `core.permissions` (or imports from there) | [D3-time-tracking-permission.md](D3-time-tracking-permission.md) |

**Count:** 13 items in scope. Wave 1 = 8 items (A1, A2, A4, A5, A6, B1+B2, F2, D3). Wave 2 = A3, A7, F1, F3. F4 deferred to Week 2 (Group C dependency).
...
- **Branch:** `code-review-fixes-2026-04-23`
- **Wave:** 1 (specs being drafted)
