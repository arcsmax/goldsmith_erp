...
### The bench batch — 16 commits landed after this document's `aed469d` checkpoint

This document's changelog above was written as of `aed469d` (2026-09-25 17:00). A further batch of
16 non-merge commits, `3713c0a`..`1a3bbe9`, landed on `audit/2026-09-fixes` afterward, fixing four
problems reported directly from bench use rather than from the original 212-finding register:
configurable Standorte (workshop locations), editing a running timer, QR/HID scanner tracking, and
the allergy-consent gate in the consultation wizard. Two more commits in the same run,
`700b9b6`/`7ca9473` (LV2-06/LV2-05/LV2-03), close open follow-ups already tracked in section (c)
below and are noted there, not repeated here. Source: `fix-locations.md`, `fix-timer-edit.md`,
`fix-timer-locations.md`, `fix-nogo-consent.md`, `fix-scanner-hid.md`, `fix-scanner-tracking.md`
and `scanner-diagnosis.md` in `.orchestrated-fable/ux-erp-audit-2026-09/`, plus `git show --stat`
on each commit.

### W8 — Standorte: configurable workshop locations — **partial**
- No register finding (bench-session report, not part of the original 212). Commits: `3713c0a` (`workshop_locations` table + migration `20260925_w8_workshop_locations`, seeded from the distinct legacy free-text values already in use), `e0279dd` (Standorte API — `GET /locations`, `GET/POST/PATCH/DELETE /admin/locations`, new `LOCATION_VIEW`/`LOCATION_MANAGE` permissions, write-path sync, `STANDARD_LOCATIONS` reference seed: Werkbank 1, Werkbank 2, Tresor, Ausstellung), `9b57cb2` (Standort dropdown `LocationPicker` + admin `LocationsPanel`), `59d69e0` (name-based `LocationPicker` adapter, the default-export `NameLocationPicker`, for the running-timer edit sheet — landed before the id-based picker existed), `56989a9` (regenerate frontend API types after merging `audit/2026-09-fixes`).
- `WorkshopLocation.kind` is a String+CHECK (bench/safe/showroom/external/other); `time_entries.location_id`/`orders.location_id` are nullable FKs, `ON DELETE SET NULL`, indexed, with the legacy text columns kept alongside. The migration seeds distinct trimmed strings from `time_entries.location`, `orders.current_location` and `location_history.location`, merges duplicates case-insensitively, guesses `kind` from the name, then backfills `location_id` by name; it is idempotent, and downgrade drops the FKs and table. `LocationService.resolve(location_id, name, keep_id=)` treats an id as authoritative if active (or the row's own current id via `keep_id`, so old history stays editable) and a bare name as a soft link to a matching location if one exists, else kept as free text — used by timer start, manual time-entry create/update, order update and `POST /orders/{id}/location`. ADMIN gets a "Standorte" panel on the Systemübersicht (add, rename via `PromptDialog`, reorder, deactivate via `ConfirmDialog`, reactivate); the picker itself is a `Select` over active locations with an ADMIN quick-add ("Standort hinzufügen"), wired into `TimeEntryFormModal`, `ArbeitszettelTab` (the hardcoded `LOCATION_OPTIONS` removed) and the order form's `BasicSection`.
- Left open (`fix-locations.md`): at the time of `59d69e0`, the running-timer-edit service wrote only the free-text `location` column, not `location_id` — closed separately by `fix-timer-locations.md`'s `b8142c7`/`4b5ea30`, see the next entry; the scanner's device-location switch (`switch_timer`) still stores text only, `location_id` stays NULL — scanner scope, confirmed still open by `fix-scanner-tracking.md`; `OrderCreate` still has no `current_location`/`location_id` field, so a newly created order ignores Standort until its first edit; legacy slug values (e.g. `werkbank_1` from the old Arbeitszettel options) are seeded verbatim as location names — ADMIN can rename them in the panel, but nothing does so automatically; the mypy strict ledger was not re-run for `time_tracking_service.py`, `order_service.py` or `models/order.py` (only the new modules were mypy-checked); the 1280/390 screenshot loop was not run (no live stack in the worktree).
- Evidence: `tests/unit/test_migration_w8_workshop_locations.py`; `tests/integration/test_locations_api.py` (new, 348 lines); full backend suite 4449 passed, 12 skipped, 1 xfailed after the merge; frontend `vitest run` 1387 passed; `tsc --noEmit` 0; `yarn lint` 0 errors; `hex-ratchet` OK 35/35.

### Running-timer edit: Standort resolution and id-based picker — **partial**
- No register finding. Commits: `38d0497` (new feature — `PATCH /time-tracking/{entry_id}`, `services/running_timer_edit.py`, `RunningTimerEditSheet.tsx`), `b8142c7` (`resolve_location()` wrapper: an unknown Standort **name**, not just an unknown id, now 422s on timer start and running-timer edit instead of silently falling back to free text; `LocationService.resolve` itself was left untouched and stays tolerant for the order/manual-entry callers), `4b5ea30` (id-based `LocationPicker` on `TimerStartForm` and the edit sheet, replacing the interim name-based adapter from `59d69e0`; remembers the device's last-used Standort in `localStorage` via new `lib/deviceLocation.ts`).
- `RunningTimeEntryEdit` (activity, job/order, location, notes, start time) is `extra="forbid"` and requires at least one field. `services/running_timer_edit.py` enforces start-time bounds with German 422 messages (`start_in_future`, 1 min tolerance; `start_too_old`, over 24h; `start_before_previous`, before the max `end_time` of the user's other entries), diffs a no-op edit into a no-write/no-event, and keeps an append-only change log in `notes` below a `--- Änderungsprotokoll ---` marker (user id only, no PII), since there is no dedicated `edit_log` column. Owner-or-ADMIN only. `TimerWidget` gets a "Bearbeiten" button opening a `Sheet` with Activity/Job/Location pickers (each its own "… ändern" button), a start-time field and a notes field; `JobPicker` is a new paged typeahead over `GET /jobs?kind=order`.
- Left open (`fix-timer-edit.md`, `fix-timer-locations.md`): repairs cannot be picked in the edit sheet — `time_entries.order_id` is NOT NULL with an order-only FK and no repair link exists, out of scope; the audit trail lives in the `notes` text field, not a tamper-proof column, and an ADMIN using the PUT endpoint could still overwrite it wholesale; the start-time field only supports the timer's own calendar day (crossing midnight needs a date field); a new start time is not checked against open interruptions that began before it; `create_time_entry`/`update_time_entry` (manual entries, PUT) deliberately stayed on the tolerant free-text `LocationService.resolve`, not the strict 422 path; the 1280/390 screenshot loop was not run.
- Evidence: `tests/integration/test_edit_running_timer.py` (13 tests, plus new `TestEditRunningTimerLocationResolve` cases from `b8142c7`); `frontend/src/components/time-tracking/RunningTimerEditSheet.test.tsx`; `frontend/src/components/time-tracking/TimerStartForm.test.tsx` (new); full backend suite 4478 passed, 12 skipped, 1 xfailed after the final re-merge; frontend `vitest run` 1423 passed (152 files); `tsc --noEmit` 0; `yarn lint` 0 errors, 81 pre-existing warnings.

### SC-01 to SC-06 — QR/HID scanner: every scan logged, per-piece history, admin search, global USB wedge — **partial**
- No register finding (SC-01 through SC-10 come from a same-session read-only diagnosis, `scanner-diagnosis.md`, not the original 212-finding register). Commits: `b53ffd3` (scan-history read side: `GET /orders/{id}/scans`, `GET /repairs/{id}/scans`, `scan_updates` realtime channel — SC-02, SC-03), `1e15d09` (log every scan before the action sheet opens, Sheet-based scan actions — SC-01, SC-05, SC-06), `c5b8b78` (Scan-Verlauf on orders and repairs, last-scan line, scan-history search — SC-02, SC-03), `2877229` (scan location from the W8 workshop locations — SC-04, partial), `629baba` (connect the USB/keyboard-wedge scanner globally in the authenticated shell), `491c79b` (Mitarbeiter:in filter dropdown on the admin scan-history panel), `1a3bbe9` (lint cleanup in `HidScannerListener` + its test).
- The diagnosis found the scanner subsystem "architecturally mature but functionally disconnected": `scan_logs` and its endpoints already existed, tested, but no live scan path ever called `POST /scan/log` — scanning a piece of jewellery wrote nothing to the log (SC-01, its highest-priority finding). Fix: every decode (camera, typed number, hand-scanner keystrokes typed into the input, a "Letzte Scans" re-open) now POSTs `/scan/log` before the action sheet opens (`scan_only`, `unrecognised`, or `resolve_failed`); a picked action writes a second append-only row linked by `parent_scan_id` with its own `action_result`; failed POSTs queue in `localStorage` (capped at 100) and flush via `/scan/log/batch`. New `GET /orders/{id}/scans`/`GET /repairs/{id}/scans` (paged, newest first, VIEWER-safe projection) back a "Scan-Verlauf" card and a "Zuletzt gescannt von … um … in …" line on order/repair detail (SC-02). New `GET /scan/history?q=&user=&from=&to=&limit=&offset=` (ADMIN/GOLDSMITH, 403 for VIEWER, a structured audit log line per search) backs `pages/admin/ScanHistoryPanel.tsx`, mounted on the ScannerPage, now with a "Mitarbeiter:in" `<select>` scoped to ADMIN only — GOLDSMITH lacks `USER_VIEW` so would 403 on `GET /users/`, and keeps its existing free-text "Nummer oder Code" search unchanged (SC-03). Device location is chosen once per device from the W8 Standort dropdown (`DeviceLocationSetting`, `LocationPrompt`) and carried in the scan's whitelisted `context` JSON (no `scan_logs` migration), resolved server-side to a name via `LocationService.resolve`; an unknown or inactive id is dropped rather than losing the scan (SC-04, partial). A single payload grammar (`lib/scanPayload.ts`) is cross-checked against the backend's `KNOWN_PREFIXES_V1_1` by `tests/unit/test_scan_prefix_sync.py` (SC-06). Two extra bugs were fixed along the way: ScannerPage's own scans reopened the camera instead of the action sheet (the overlay's open effect was resetting the result before the page could hand it off); and the backend offered actions with no client handler (`add_material`, `add_note`, `contact_customer`, `repair_diagnosis`, `check_stock`, `reorder`, `advance_repair`), surfacing "Unbekannte Aktion" on tap — the action registry is now curated and `advance_repair`/`handover`/`log_only` handlers were added. Separately, `HidScannerListener` (new) wires `ScannerContext`'s existing keyboard-wedge burst detector into `App.tsx`: it was already armed by the "Werkbank-Station-Modus" toggle and exposed an `onBenchScan` injection point, but nothing supplied a real handler, so a USB scan outside a focused text input was silently dropped on every page except ScannerPage; it is still gated by that same per-device toggle, just now reachable from anywhere in the authenticated shell instead of unreachable everywhere.
- Left open: **no migration yet for an indexed `scan_logs.location_id` column** (or a `(resolved_type, resolved_id, scanned_at)` index for per-piece lookups at volume) — SC-04's remainder, per this batch's own handoff summary said to be **in progress in a following session**; SC-08 (a `Permissions-Policy: camera=...` header) untouched, infra, not code in this repo; SC-09/SC-10 (alias-resolution stub, a scan-log retention/purge job) out of scope; `ScanHistoryPanel` is still only reachable from `ScannerPage`, not linked from `AdminSystemPage`; no real PostgreSQL pass exercised the partitioned `scan_logs` INSERT path in this batch (SQLite only); no scanner e2e spec exists in the repo (`fix-scanner-tracking.md` searched for one and found none); the 1280/390 screenshot loop was not run for any of this batch's UI (Scan-Verlauf card, ScanHistoryPanel filter, action sheet) — **left open per the batch's own handoff as a 390px visual check still to do.**
- Evidence: `tests/integration/test_scan_tracking.py` (21 tests after `b53ffd3`/`1e15d09`/`c5b8b78`, +more after `2877229`); `tests/unit/test_scan_prefix_sync.py`; `frontend/src/test/ScanTracking.test.tsx` (17 tests); `frontend/src/test/ScanHistoryViews.test.tsx` (7); `frontend/src/test/HidScannerListener.test.tsx` (2); `frontend/src/test/ScanHistoryPanelUserFilter.test.tsx` (3); full backend suite 4472 passed after the scan-tracking merge, `yarn lint` 0 errors, `hex-ratchet` 35/35; after the HID/filter merge, full backend suite 4478 passed, 12 skipped, 1 xfailed, frontend `vitest run` 1424 passed (153 files), `yarn lint` 0 errors, `vite build` 0.

### Allergy No-Go consent block in the consultation wizard
- No new register finding — this closes a residual gap in the already-`fixed` GDPR-02 (W1-05): the backend's Art. 9 consent gate was correct, but one frontend entry point never surfaced it. Commit: `dcd298f`.
- Bug: clicking a "Schnellauswahl Allergien" chip (Nickel, Kupfer, Silber) in the consultation wizard's No-Go step (`/consultations/{id}?step=4`) showed a generic "No-Go konnte nicht angelegt werden" toast. Confirmed by a new integration test that `POST /customers/{id}/no-gos {"category":"allergy",...}` already correctly returned 422 with the real German consent-required message when no `HEALTH_DATA` consent existed, and 201 after granting it — so the backend needed no change. Fix: new `HealthDataConsentBlock.tsx` ("Einwilligung Gesundheitsdaten", mirroring `ConsentPanel.tsx`'s grant form) renders above the quick-allergy chips in `StyleNoGoStep.tsx`; the three chips and the manual add-form's submit button are disabled until an active `HEALTH_DATA` consent exists, with an inline hint on the manual form; the catch-all toast now shows the server's real message via `getErrorMessage` instead of a hardcoded string. Non-health No-Go categories (e.g. `metal`) are unaffected — no consent check, no gating.
...
**The bench batch (new open items, commits `3713c0a`..`1a3bbe9`, after this document's `aed469d` checkpoint):**

| Item | Source report | Severity guess | Suggested wave |
|---|---|---|---|
| No indexed `scan_logs.location_id` column (or a `(resolved_type, resolved_id, scanned_at)` index) — SC-04's remainder; **the batch's own handoff says this is in progress in a following session, do not treat it as done** | fix-scanner-tracking.md, fix-locations.md | M | in progress (parallel session) |
| 1280/390 screenshot loop never run for the bench batch's own UI (Standorte panel, running-timer edit sheet, scan action sheet, ScanHistoryPanel filter) — **left open per the batch's own handoff as a 390px visual check still to do** | fix-locations.md, fix-timer-edit.md, fix-timer-locations.md, fix-scanner-hid.md, fix-scanner-tracking.md | M | needs live browser verification |
| Scanner's device-location switch (`switch_timer`) still stores location as text only; `location_id` stays NULL | fix-scanner-tracking.md | M | scanner follow-up |
| Running-timer edit sheet cannot pick repairs — `time_entries.order_id` is NOT NULL with an order-only FK, no repair link exists | fix-timer-edit.md | M | needs a schema change |
| Timer-edit audit trail lives in `notes` text (`--- Änderungsprotokoll ---` marker), not a tamper-proof column; an ADMIN PUT could still overwrite it wholesale | fix-timer-edit.md | L | hardening |
| A new start time on a running-timer edit is not checked against open interruptions that began before it | fix-timer-edit.md | L | not stated |
| `OrderCreate` still has no `current_location`/`location_id`; a new order ignores Standort until its first edit | fix-locations.md | L | not stated |
| Legacy slug location values (e.g. `werkbank_1`) seeded verbatim as Standort names; ADMIN can rename but nothing does so automatically | fix-locations.md | L | cleanup |
| `ScanHistoryPanel` still only reachable from `ScannerPage`, not linked from `AdminSystemPage` | fix-scanner-tracking.md, fix-scanner-hid.md | L | not stated |
| No scanner e2e spec exists in the repo | fix-scanner-tracking.md | M | test coverage |
| Real PostgreSQL pass not run for the partitioned `scan_logs` INSERT path in this batch (SQLite only) | fix-scanner-tracking.md | M | pre-deploy verification |
...
**A bench batch landed after this checkpoint was written.** 16 further commits
(`3713c0a`..`1a3bbe9`) fixed four problems reported from actual bench use —
configurable Standorte/workshop locations (W8), editing a running timer,
QR/HID scanner tracking (SC-01 through SC-06), and the allergy-consent gate in
the consultation wizard — see the new changelog entries in section (a) and the
new open-items table in section (c). That batch's own handoff summary claimed
a final gate of **backend 4478 tests / frontend 1428 tests / `yarn lint` 0
errors — these are numbers *reported by the batch*, not independently
re-verified for this checkpoint**, consistent with this document's own rule
(reports first, commits second, never invent a status). Two items were left
explicitly open per that same handoff: a 1280/390px visual check of the new
UI, and an indexed `scan_logs.location_id` column, said to be **in progress in
a following session** — treat both as outstanding, not done, until confirmed.
