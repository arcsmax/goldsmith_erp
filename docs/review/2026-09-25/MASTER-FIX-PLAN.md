...
- **Integration branch:** `audit/2026-09-fixes` (created off `main` @ `73fff19`; PR #51 (draft) is the review vehicle; ~53 fix-item branches plus 2 live-verification rounds merged in as of 2026-09-25). **Wave 1 is complete** except the items marked `partial` below (W1-04, W1-05, W1-07, W1-08, W1-10, W1-12, W1-14, W1-15, W1-18); every other Wave 1 item is `done`. **Wave 2 is complete**: every item has landed, several `partial` (W2-03, W2-04, W2-06, W2-09, W2-11, W2-12, W2-13, W2-16) and the rest `done`. **Wave 3** is mostly landed: W3-02, W3-06, W3-07, W3-08, W3-10 are done or partial, W3-09 has not started; **W3-11 (the Numeric/tz migration) is still running (`in progress`)** as this pass is written; W3-01, W3-03, W3-04, W3-05 have not started. **Wave 4**: W4-01 (tokens) is partial and a `src/ui` primitive library landed under the W4-02 commit label (see its note below for a numbering caveat); W4-04 through W4-10 (shell/nav, per-page migration, customer-facing surfaces, cleanup, dark mode) have not started. **Wave 5** is mostly landed: W5-01, W5-02, W5-03, W5-05, W5-07, W5-08, W5-09, W5-10, W5-11, W5-13 done or partial; W5-04, W5-06, W5-12 have not started. **Wave 6**: W6-01 (outbox), W6-02 (CustomerMessage) and W6-06 (status-report PDF) are done; W6-03, W6-04, W6-05, W6-07 have not started. **Wave 7**: only W7-06 has partial coverage (backend/seed/theme hygiene landed outside its own file list); W7-01 through W7-05 have not started. A further agent was still working the open items that surfaced after Wave 2 landed (dedupe/interaction fixes; folded into the W2-02 note below) as this pass was written. See [FINDINGS-REGISTER.md](FINDINGS-REGISTER.md) for the per-finding detail behind every wave.
...
**Status (as of 2026-09-25).** Wave 2 is complete: every item has landed.

- **W2-01** — done. FE-13, DOM-01 (`2d1a92d`, `342ac5a`, `f5cf713`, `8f8e1b9`); scan-to-Fotos-tab deep link closes FE-04's photo action. Still open: `?edit=location`, `?action=print-label`, `?action=consume-material` deep links unhandled; no `order_updates` publish on photo upload (fixed later by W7 hygiene, `7287033`).
- **W2-02** — done. DOM-12 (`664231b`): repairs get a draft-then-one-tap-send Kundeninfo update instead of a false `customer_notified_at`. Also closed, mid-item, the adversarial C1.2 x C2.2 interaction bug where the occurrence-scoped dedupe check and the DB-level unique dedupe key disagreed (`718791a`).
- **W2-03** — partial. FE-05, DOM-14, DOM-15, DOM-15b done via the new `GET /dashboard/today` Heute view (`9bb29ba`, `f6b4c57`); DOM-15c (receivables/overdue-invoices glance) explicitly not built.
- **W2-04** — partial. BE-16, DOM-24b done; DOM-24 partial (`4286fc2`: WorkshopSettings/NumberSequence back gap-free numbering and full §14 UStG invoice content, but `templates/invoice.html`'s preview wasn't updated to the new layout, and issuing with incomplete Werkstatt-Stammdaten only warns rather than blocking). D-12's Auftragsnummer is not built (counter service is ready for it).
- **W2-05** — done. DOM-11, DOM-11b, DOM-11d, DOM-03, DOM-09 (`34d1cfb`, `ef3b1f8`); closes the adversarial round's BE-17 gaps (A3.1 zero-price guard, A3.3 CAS guard against concurrent double-conversion, A3.4 customer-match recheck). FE-18 stays partial: `/quotes` still ignores `?quote_id`, the dashboard's quote row opens the list not the specific quote.
- **W2-06** — done (bundled with W2-11/14/16 in one report). DOM-04 (partial), DOM-05, DOM-06, DOM-09 (`a57beac`). Left open: gemstones not wired into quote/invoice PDF rendering, a new order can't add stones until saved, legacy 333/900 gold missing from the alloy picker.
- **W2-07** — done. ARCH-01, BE-06, DOM-13, DOM-46 (`e5511ef`, `34d1c6d`, `4acfa8d`): transition table, `order_events` audit table, `PATCH /orders/{id}/status`, `GET /orders/{id}/timeline`. Left open: `quote_service.convert_quote` still creates CONFIRMED orders directly, bypassing the new transition table; migration not exercised on PostgreSQL.
- **W2-08** — done. DOM-16, DOM-17, DOM-18, DOM-30 (`962edbd`, `487d3e2`, `b033057`, `1aa8f0b`, `72b9e9e`): order page collapsed from up to 14 tabs to 5, "Weiter" button, real `/timeline`-backed history, milestone-prompted Kundeninfo drafts. Status label maps stay duplicated across pages pending the Wave 4 `src/design/status.ts`.
- **W2-09** — partial. DOM-22, DOM-23 done (`70d799b`, `3938c25`): Feingehalt vocabulary driven by alloy, "nicht punziert: <Grund>" path, tightened soft gate. DOM-44 confirmed still open (`frontend/src/api/hallmarks.ts`'s orphaned OrderHallmark client left in place, dead). The report's own post-merge full backend suite run did not complete in-session and needs re-verification.
- **W2-10** — done. DOM-02, D-11 (`16eb65a`): a customer needs only one of email/phone/mobile. CSV customer import still requires an email column.
- **W2-11** — partial. DOM-34 done, DOM-35 partial (`f6f5628`): Abholprotokoll PDF and a Wertgutachten button on delivered orders, with care instructions; the care text is hardcoded, not workshop-editable.
- **W2-12** — partial. FE-17 done, DOM-38 partial (`470d3b0`, `3b91696`, `3340086`, `49e2efd`): single-screen counter intake with an Annahmeschein PDF, and a customer Verlauf tab merging orders/repairs/quotes/invoices/updates. DOM-08's signature capture explicitly not built (no schema change made); consultations still missing from Verlauf; invoices still capped at 200 there.
- **W2-13** (live updates) — partial. The hub, notifications and time-tracking realtime wiring landed (`c8065f8`, `faa106e`), closing FE-08, BE-20 (partly) and, as a side effect, the adversarial round's CRITICAL D.1 `/ws/orders` price leak. Still open: Dashboard/Orders/OrderDetail/Repairs pages don't yet call `useRefetchOn`, and `repair_updates`, `material_updates`, `consultation_updates`, `metal_price_updates` and `anomaly_alerts` have no subscriber (repair_updates confirmed still unwired even after the later W7 hygiene pass).
- **W2-14** — done. BE-19 (`93889f8`): interruption minutes now reduce time, `actual_hours` recomputed. TimerWidget's server-side pause/resume UI (D-15) explicitly not built.
- **W2-15** (metal price feed) — done. BE-22, DOM-11c (`60a8289`); minor open item: the estimator's alloy-override input has no debounce.
- **W2-16** — done. DOM-21 (`abd162c`): Altgold ID capture and Ankaufsbuch export above the cash threshold. Left open: GDPR Art. 15 export/retention of the captured ID fields, no admin export UI, `total_fine_gold_g` still aggregates across metals (needs a schema change, tracked from W1-08).
...
- **W3-01** — open. No report addresses the public/private shell split beyond what W1-13 already did, or the dead-code removal.
- **W3-02** — done. ARCH-07, FE-12 (`f532896`, `d0cbb13`, `8dff361`): generated `schema.d.ts` from the live OpenAPI spec, wired into a CI drift gate; fixed the role/notification/calendar-event enum casing bugs. Several input (Create/Update) types remain hand-written.
- **W3-03** — open. No agent has started TanStack Query adoption.
- **W3-04** — open as its own item, though the Wave 4 `src/ui` primitives (landed under the W4-02 label) build overlapping Modal/PageState/labels pieces ahead of it.
- **W3-05** — open.
- **W3-06** — partial. ESLint 9 with react-hooks and jsx-a11y rolled out and gating `yarn lint`/CI `lint-frontend` (`445b5bd`, `1f0c253`, `ee852ed`), closing issue #33; 5 errors remain in `OrderFormModal.tsx` (owned by another in-flight agent), the sole remaining blocker to a green lint gate.
- **W3-07** — partial. `DomainError` hierarchy with one handler emitting `{detail, code, extra}` landed (`5183407`); 63 router except-blocks still hand-map errors instead of raising it.
- **W3-08** — partial. `Page[T]` envelope with server-side filter/search live for orders, repairs, quotes, materials, time-tracking, notifications, capped at `limit<=500` (`7e48494`, `8dff361`), closing SEC-16; the frontend doesn't send `offset` or consume the envelope yet, and customers/invoices/activities/comments/users/calendar endpoints remain unpaginated.
- **W3-09** — open. Dead repository layer (ARCH-03, BE-24) and routers running raw SQL are untouched; `list_queries.py` now duplicates OrderService/RepairService filter logic on top of that.
- **W3-10** — partial. One Redis subscriber per process landed as part of W2-13 (`c8065f8`); lifespan-managed task cancellation, rejecting `?token=` query auth, `is_active`/Origin checks on the WebSocket, and moving SMTP config off in-process state are all still open.
- **W3-11** — in progress. The Numeric/tz migration agent is still running as this pass is written. From earlier waves, `InvoiceService.calculate_totals` is already Decimal/ROUND_HALF_UP and `UtcNaiveDatetime` normalises a handful of columns (invoice due/paid dates, time-entry end_time); the ~70-column Float-to-Numeric money migration and the ~98-column naive-to-aware timestamp sweep are not yet landed. `QuoteService.calculate_totals` still uses float and `round()`.
...
- **W4-01** — partial. Semantic tokens, AA-contrast primary (`#b45309`), global `:focus-visible` ring overriding 35 `outline:none` rules, and full tone coverage for all 10 order + 9 repair statuses landed (`54f9c3b`, `316935f`); the backend theme default and a server-side contrast gate on `PUT /theme` followed later (`7c30827`, filed under W7 hygiene). Left open: ~20 umlaut strings (DES-17) partially addressed elsewhere (see the register), hardcoded hex in `order-detail.css`/`dashboard.css`, legacy z-index literals not remapped, no axe smoke run (`@axe-core/playwright` not installed), focus ring shipped at 2px vs the playbook's 3px.
- **W4-02** — **numbering caveat:** the fix agent that ran under this label built the `src/ui` component-primitive library (Button, IconButton, Card, Modal/Dialog/Sheet, PromptDialog, Field, DataTable, ListCard, EmptyState, PageState, PageHeader, Tabs/TabBar, DeadlineChip, plus a dev-only `/dev/ui` demo route; 8 commits `163a781`..`a8113e8`) — this is the plan's W4-03 scope, done. The plan's actual W4-02 ask (DES-03/DES-04, status system and deadline chip) is separately covered: DES-03 by the earlier `StatusBadge`/status map (`dc83d95`) plus the W4-01 status-tones sweep (`54f9c3b`); DES-04 (deadline urgency) has a `DeadlineChip` primitive but no confirmed adoption into Orders/Dashboard, so it stays open. None of the new primitives are yet adopted by existing pages — that migration is W4-05 through W4-07.
- **W4-03** — see the W4-02 note: the primitive library, an inline icon-set stand-in, and EmptyState/PageState (covering part of DES-18/DES-19) landed under the `W4-02` commit label. Not yet migrated into existing pages.
- **W4-04** through **W4-10** — open. No report addresses shell/nav grouping, the bottom tab bar, per-page migration (Orders, Scanner, Time tracking, Customers, the rest), customer-facing surfaces, duplicate-class cleanup/Tailwind decision, or dark/high-contrast mode.
...
- **W5-01** — partial. Backend/frontend Dependabot critical/high alerts cleared (`94a272f`); frontend coverage tooling added as part of the same pass (`40fe1a4`). The `ecdsa` transitive dependency needed finishing the python-jose to PyJWT migration, closed later by W7 hygiene (`250e5a9`). `docs/architecture/likec4/package-lock.json` stays vulnerable — a separate Node project, out of scope.
- **W5-02** — done. Nightly E2E workflow for the 4 previously-unwired specs, advisory pip-audit/yarn-audit, ruff in the CI lint job (994-error baseline, non-blocking), Poetry/Yarn caching (`23fd91c`, `a3642f1`, `533e62d`). Left open: OPS-03's mypy widen-without-shrink gate needs dedicated design work, OPS-10's test relocation not done, coverage has no enforced threshold yet.
- **W5-03** — done. Systemd compliance-timer installer and local `make test`/`make lint` targets (`33cce19`, `533e62d`). Left open: the systemd cleanup unit's default `COMPOSE_FILE` still points at the dev compose file, not the prod one.
- **W5-04** — open. No real-time contract test added.
- **W5-05** — done. Encrypted (age) backups, two pre-existing retention-loop bugs fixed (oldest/newest inversion, a `set -e`-fatal counter bug), a restore-then-replay erasure ledger and CLI (`3f0e7f7`). Left open: no live restore drill performed, `restore.sh` lacks `ON_ERROR_STOP`, `backup-sync.sh` is still URL-only auth, `./uploads` not backed up.
- **W5-06** — open. No column-encryption or key-rotation-path work landed.
- **W5-07** — done. Retention schedule and a `RETENTION_EXECUTE` gate, dry-run by default (`15bd0c0`). Sweep coverage not yet extended past its original tables to inactive customers, consultations, quotes, notifications, audit logs.
- **W5-08** — done. Full Art. 15 export (`7b1d756`). D-13's design-IP export scope is still pending lawyer confirmation.
- **W5-09** — partial. Processors list documented (`537594a`); employee-analytics Art. 30 entry needs Anne's decision (GDPR-14); TLS-on-every-SMTP-port enforcement (GDPR-15) not implemented.
- **W5-10** — done. VVT v1.2 (controller Anne, processor Max under an AVV), TOMs, Art. 13 customer notice, AVV checklist, breach runbook (`863b38c`). Several sign-offs (retention periods, DPO/AVV structure, breach thresholds, TOMs adequacy) still pending Anne, a lawyer or a Steuerberater.
- **W5-11** — partial. Stale `ARCHITECTURE_REVIEW.md`/`DEPLOYMENT.md` archived with superseded banners, rollback and log-rotation documented, a single-box deployment ADR added, the demo-password and SameSite=Strict-on-an-IP-site limitations documented (`03bf8ac`, `2d748f6`). The C4/likec4 model (tool itself fails to parse) and the jose/estimator/no-portal ADRs are not done.
- **W5-12** — open. No error-tracking (Sentry/GlitchTip) wiring or one-shot migrate service landed; the new outbox admin panel (W6-01) gives some queue observability as a side effect.
- **W5-13** — done. A read-only stale-worktree/branch reporter that never auto-deletes (`c3df1a4`).
...
- **W6-01** — done. Transactional `outbox_messages` table (lease/backoff/dead-letter via `FOR UPDATE SKIP LOCKED`), a `worker.py` process, an admin retry endpoint and Nachrichten-Warteschlange UI panel, and mode selection (inline in DEBUG, worker otherwise) (`c4e4090`, `d41625b`, `f55fbaf`, `1409dc8`, `d099e0e`, `30e63c0`). Left open: no invoice-email path through it yet, a queued message shows as delivered until the worker actually sends, delivery is at-least-once (possible resend if a lease expires mid-crash).
- **W6-02** — done. `CustomerMessageService` is the single outbound customer path, policy-driven per message kind, enforcing PHOTO_USE consent, a price guard, an Art. 21 opt-out to PDF_MANUAL fallback and an audit row per send (`777137d`, `815196d`, `2ab80c0`); quote e-mails routed through it too (`2669451`), closing the gap where `quote_delivery.py` originally bypassed it. Left open: E7's recipient-confirmation step before the first photo mail (needs frontend UI), and a retry-via-generic-`/updates/{id}/send`-endpoint edge case that can mis-derive the message kind.
- **W6-03** — open. No `media_assets`/`MediaStore` consolidation landed; the three photo tables (order, repair, consultation) remain separate.
- **W6-04** — open. Order and RepairJob remain parallel aggregates; repairs still cannot be invoiced.
- **W6-05** — open. `db/models.py` and the services are not modularised.
- **W6-06** — done. Customer status-report PDF for orders and repairs, gated a second time on active PHOTO_USE consent at generation time, with a composer "Statusbericht anhängen" option and standalone download buttons (`c4c1ff5`). Left open: repair photos have no consent/selection mechanism yet (latest 6 shown unconditionally), `next_steps` is a query param not wired into the composer as free text, no `RepairEvent` history table backs the repair timeline.
- **W6-07** — open. No token status page, portal link rendered in customer emails, or approve/decline-in-one-click flow built.
...
- **W7-01** through **W7-03**, **W7-05** — open. No workshop-model (assignee/apprentice/multi-piece/QC checklist), aftercare/deposit, offline-queue, or audit-decorator work landed.
- **W7-04** — open. No feature-flag gate added; the unused `boto3` dependency is confirmed dead but not removed.
- **W7-06** — partial. Work addressing the same "prior-audit hygiene without IDs" intent landed outside this item's own file list: python-jose replaced with PyJWT repo-wide, closing the `ecdsa` vulnerability (`250e5a9`); demo/seed credentials unified across every seed script and 3 previously-hardcoded Playwright specs (`4b2d1ec`); backend theme defaults and a server-side contrast gate added (`7c30827`); a photo-upload realtime-publish gap and a `CustomerDetailPage` thumbnail N+1 fixed as W2-01 follow-ups. FE-25 itself (`yarn build` exits 127 in this checkout) remains open — every fix agent still works around it via `node node_modules/vite/bin/vite.js build`. Note: during this work a message purporting to be "the coordinator" asked the hygiene agent to also fix LV-05/16/18; the agent flagged it as a likely prompt injection, declined, and stayed in its assigned scope — worth a human read of that report before trusting anything claiming to expand an agent's scope mid-task.
