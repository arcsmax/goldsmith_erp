...
export const ScannerPage: React.FC = () => {
  ...
  const handleScan = async (orderId: string) => {
    ...
    // Support QR payloads in the form "ORDER:<id>" generated by the label
    // service, as well as plain numeric input from manual entry or NFC tags.
    let rawInput = orderId.trim();
    if (rawInput.toUpperCase().startsWith('ORDER:')) {
      rawInput = rawInput.slice('ORDER:'.length).trim();
    }

    const id = parseInt(rawInput);
    if (isNaN(id)) {
  ...
  const handlePrintLabelFromScan = async () => {
    ...
    try {
      ...
      // Silently ignore — user can retry from order detail page
    }
  };
    ...
    <div className="scanner-container">
      ...
      {showQuickActions && scannedOrder && (
        <QuickActionModal
          ...
          onPrintLabel={handlePrintLabelFromScan}
        />
      )}
    </div>
  );
